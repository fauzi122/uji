...
class InputmanualController extends Controller
{
    ...
    public function kelas_input_manual($id)
    {
        ...
        // 1 => "AFS.22.4B.12.A"
        // 2 => "17:00-19:30"
        // Dekripsi ID dan parsing nilai
        $exp = explode(",", Crypt::decryptString($id));

        // Cek jika jadwal adalah gabungan
        ...
        if ($cek > 0) {
            ...
            // Jika kelas bukan gabungan (praktek atau lokal)
        ...
        // Ambil data dosen pengganti berdasarkan kode mata kuliah dan kelas lokal
        $dosenPenggantiList = DB::table('dosen_pengganti')
            ->join('karyawanbs1', 'dosen_pengganti.kd_dp', '=', 'karyawanbs1.kd_dosen')
            ->select('dosen_pengganti.kd_dp', 'karyawanbs1.nama', 'karyawanbs1.gelar', 'karyawanbs1.sex', 'karyawanbs1.nip')
            ->where('dosen_pengganti.kd_mtk', $exp[0])
            ->where('dosen_pengganti.kd_lokal', $exp[1])
            ->where('dosen_pengganti.jam_t', $exp[2])
            ->groupBy('dosen_pengganti.kd_dp')
            ->get();

        // dd($dosenPengganti);
        return view('administrasi.inputmanual.rekapinputmanual', compact('jadwal', 'absen', 'id', 'dosenPenggantiList'));
    }
