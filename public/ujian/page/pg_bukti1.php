...
if(!isset($_POST['userid']) or !isset($_POST['testid'])){
	...
	<?php
		...
		$q_bukti=mysql_query("SELECT a.resultid,a.userid,c.user_firstname,c.user_email,c.user_name,a.testid,
								b.test_name, b.test_attempts,a.result_endtime, SEC_TO_TIME(a.result_timespent) AS spent_time, 
								...
								a.result_pointsmax,
								RIGHT(FROM_UNIXTIME(a.result_datestart),8) AS awal1,
								FROM_UNIXTIME(a.result_datestart) AS awal,
								RIGHT(FROM_UNIXTIME(a.result_dateend),8) AS akhir1,
								FROM_UNIXTIME(a.result_dateend) AS akhir,
								FROM_UNIXTIME(a.result_endtime) AS akhir2
								...
								AND (b.jmlsoal <=
  (SELECT
    COUNT(result_answerid)
  FROM
    ".$db_prefix."results_answers AS d
  WHERE d.resultid = a.resultid)
  or 
  b.jmlsoal >=
  (SELECT
    COUNT(result_answerid)
  ...
  WHERE d.resultid = a.resultid))");
		...
		$list_num=mysql_num_rows($q_bukti);
		$q_test=mysql_query("SELECT waktu_habis FROM ".$db_prefix."tests_attempts WHERE userid='$nim' AND testid='$test_id' ");
		$waktu_habis=mysql_fetch_array($q_test);
		if($list_num<1){
		?>
			<script language='javascript'>document.location = '<?php echo $path_base."-".$s_id;?>';</script>
			<meta http-equiv="Refresh" content="0; URL=<?php echo $path_base."-".$s_id;?> ">
		<?php
			...
			// Cek test_attempts //
			$attempts=mysql_query("SELECT test_attempt_count FROM ".$db_prefix."tests_attempts WHERE userid='$nim' AND testid='$test_id' ");
			$test_attempts=mysql_fetch_array($attempts);
			$num_attempts=mysql_num_rows($attempts);
			if($list_bkt['result_endtime']<$unix_timestamp){
		mysql_query("INSERT INTO ".$db_prefix."tests_attempts
				...
				mysql_query("update ".$db_prefix."visitors	set 
								  hits = '$test_id'
								where userid = '$nim' AND LEFT(FROM_UNIXTIME(startdate),10)='$tgl_sekarang' AND hits=0 ");
			}elseif($test_attempts['test_attempt_count']<$list_bkt['test_attempts'] and $test_attempts['test_attempt_count']<$list_num){
			...
			// End Cek test_attempts //
			if($waktu_habis['waktu_habis']=='1'){
			$awal=date_create($list_bkt['awal']);
			$akhir=date_create($list_bkt['akhir2']);
			}else{
			$awal=date_create($list_bkt['awal']);
			$akhir=date_create($list_bkt['akhir2']);
			}
    ...
    <div class="modal-dialog">
      <div class="modal-content">
        <div class="modal-header">
          <a type="button" class="close"  aria-hidden="true" href="<?php echo $path_base."-".$s_id;?>">&times;</a>
		...
		<tr>
			<td>Tanggal Ujian : <?php echo $datestart; if($datestart<>$dateend){ echo " s/d ".$dateend; }?>   &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Jam Ujian : <?php echo $list_bkt['mulai']." - ".$list_bkt['selesai'];?>
			
			
			</td>
		</tr>
		<tr>
		<td>Lama Waktu Mengerjakan : <?php echo $diff->h.' Jam '.$diff->i.' Menit '.$diff->s.' Detik';?></td>
			<!-- <td>Lama Waktu Mengerjakan : <?php echo $list_bkt['spent_time'];?></td> -->
		</tr>
		<tr>
		<td>Mulai Ujian : <?=$list_bkt['awal1'];  ?> &nbsp;&nbsp;&nbsp;&nbsp;&nbsp;&nbsp; Selesai Ujian : 
		<?php if($waktu_habis['waktu_habis']=='1'){
			// echo $list_bkt['akhir2'] ;
			echo $cenvertedTime = date('H:i:s',strtotime('+1 hour',strtotime($list_bkt['awal1'])));
		}else{
			//echo $list_bkt['akhir2'] ;
			echo $cenvertedTime = date('H:i:s',strtotime('+0 hour',strtotime($list_bkt['akhir2'])));
		}
			?>
		</td>
		</tr>
		...
		<tr>
			<td>Terimakasih anda telah mengikuti ujian online, 
			<?php if($list_bkt['test_attempts']>1){ ?>
			anda dapat melakukan ujian sebanyak <?php echo $list_bkt['test_attempts']; ?> Kali, 
			<?php } ?>
			bukti ujian dapat Anda cetak melaui link cetak bukti ujian dibawah </td>
		</tr>
        ...
        <div class="modal-footer">
		  ...
		  <form action="bukti-pdf-<?PHP echo $s_id;?>" method="post" style="margin:0;padding:0;" name="bukti" target="_blank">
