...
include "line/sambungan.php";
	...
	<?php
		...
		$q_bukti=mysql_query("SELECT a.userid,a.resultid, c.user_firstname,c.user_email,c.user_name,a.testid, b.jmlsoal, b.test_type,
							...
							RIGHT(FROM_UNIXTIME(a.result_endtime),8) AS akhir2
  ...
  WHERE d.resultid = a.resultid))
		...
		if($waktu_habis['waktu_habis']=='1'){
			...
			$detik=$diff->s;
		if($waktu_habis['waktu_habis']=='1'){
			$akhir1=$cenvertedTime = date('H:i:s',strtotime('+1 hour',strtotime($list_bkt['awal1'])));
		}else{
			
			$akhir1=$list_bkt['akhir2'] ;
		}
		//var_dump($akhir);exit();
...
$pdf->SetAuthor('BTIBSI');
$pdf->SetTitle('Bukti Ujian Online');
$pdf->SetSubject('Bukti Ujian Online');
$pdf->SetKeywords('Bukti, Ujian, Online');
...
<tr><td width="10%"></td><td width="20%" style="background-color:#c7c7c7;">  Mulai Ujian</td><td style="background-color:#c7c7c7;">:  </td><td width="59%" style="background-color:#c7c7c7;">$awal1 - $akhir1</td></tr>
<tr><td width="10%"></td><td width="20%" >  Lama Waktu</td><td>:  </td><td width="59%">$jam Jam $menit Menit $detik Detik</td></tr>
...
while($l_cekresultans=mysql_fetch_array($q_cekresultans)){
	if($list_bkt['test_type']==1){
		...
		$jawaban=$l_cekresultans['result_answer_text'];
$jawabhtml = <<<EOD
$jawaban 
EOD;
		...
		$pdf->MultiCell(25, 5, $bs, 1, 'C', 0, 0, '', '', true);
				$pdf->ln();
//=================JIKA JAWABAN LEBIH DARI 30 MAKA AKAN MEMBUAT HALAMAN BARU==================================//		
		if($l_cekresultans['result_answerid']==33){
