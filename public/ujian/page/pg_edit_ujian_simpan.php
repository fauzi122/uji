...
if(!empty($_POST['nomor']) AND !empty($_POST['s_answer']) OR !empty($s_answeresy)){
	...
	//Query tabel result
	$q_cekresult=mysql_query("SELECT resultid, result_points FROM ".$db_prefix."results WHERE testid='$test_id' AND userid='$nim' ORDER BY resultid DESC LIMIT 1");
	...
	if($list_test['test_type']==1){
		...
		mysql_query("UPDATE ".$db_prefix."results SET 
						...
						WHERE userid='$nim' AND testid='$test_id' AND resultid='$list_cekresult[resultid]' ");
		/// END UPDATE RESULT ///
		...
		/// UPDATE RESULT ///
		$result_points=$list_cekresult['result_points'] + $answer_correct;
		mysql_query("UPDATE ".$db_prefix."results SET 
