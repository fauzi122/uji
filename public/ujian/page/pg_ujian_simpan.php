...
//$id_user = mysql_real_escape_string(base64_decode($_GET["id"]));
	...
	$no=antiinjection($_POST['nomor']);
	$questionid=antiinjection($_POST['questionid']);
	if(isset($_POST['s_answer'.$no])){
		$s_answeresy=antiinjection($_POST['s_answer'.$no]);
	}
	
	
if(!empty($_POST['nomor']) AND !empty($_POST['s_answer']) OR !empty($s_answeresy)){
	...
	if($list_test['test_type']==1){
		...
		$s_answer=mysql_escape_string($_POST[$nosanswer]);
	}else{
		$s_answer=substr(antiinjection(trim($_POST['s_answer'])), 0,1);
	}
	...
	if($list_test['test_type']==1){
		if($row_resaswr<>0){
				mysql_query("UPDATE ".$db_prefix."results_answers SET
					result_answerid = '$no',
					 ...
					 result_answer_timeexceeded = '0',
					 ip_address = '$ipaddress',
					 result_answer_date = '$unix_timestamp'
		...
		mysql_query("UPDATE ".$db_prefix."results SET 
					...
					result_timeexceeded='0',
					result_dateend='$unix_timestamp'
	...
	?>
		<!--
			<script language='javascript'>document.location = 'bukti-ujian<?php echo "-".$s_id;?>';</script>
			<meta http-equiv="Refresh" content="0; URL=bukti-ujian<?php echo "-".$s_id;?> "> -->
	<?php
		//}
	}else{
		...
		?>
		<script language='javascript'>document.location = 'ujian<?php echo "-".$s_id;?>';</script>
		<meta http-equiv="Refresh" content="0; URL=ujian<?php echo "-".$s_id;?>">
<?php
	}
}else{
