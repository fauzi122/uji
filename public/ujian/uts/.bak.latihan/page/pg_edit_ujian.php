	...
	<?php
		...
		$q_question=mysql_query("SELECT
								...
								WHERE a.resultid=b.resultid AND b.questionid=c.questionid AND a.testid = '$test_id'
									AND a.userid = '$nim' AND b.questionid='$questionid'");
...
<div class="col-md-12">
	<div class="block-web">
		...
		<table border="0" width="100%">
			...
			<td width="98%">
				<form action='eujian.-<?php echo base64_encode($id_user);?>' method='post' id="form-ujian" novalidate="novalidate" style="width:100%;">
			...
			<?php
				...
				if($list_q['question_type']=='1'){
					...
					tinymce.init({
						...
						plugins: "table, paste, autosave, charmap",
					...
					<!--- END JAVA FOR EDITOR --->
					<br><textarea name="s_answer<?php echo $nomor;?>" class="mceEditor<?php echo $nomor;?>" style="width:100%;height:50%;"><?php echo $list_q['result_answer_text']; ?></textarea>
					<?php
				}else{ 
					...
					$q_answer=mysql_query("SELECT
											...
											WHERE a.questionid=b.questionid AND a.questionid = '$list_q[questionid]'
											ORDER BY RAND()");
					while($list_a=mysql_fetch_array($q_answer)){ ?>
						<input type='radio' value='<?php echo $list_a['answerid']; ?>' name='s_answer' class="s_answer" 
						<?php if($list_a['answerid']==$list_a['result_answer_text']){ echo "checked='checked'";} ?> /> &nbsp;<?php echo $list_a['answer_text']; ?><br>
						<?php
					}
				}
				...
				?>
					...
					<a href="edit-<?php echo base64_encode($id_user);?>" class='btn btn-primary'>Kembali</a>
				</form>
			</td>
			</tr>
		</table>
