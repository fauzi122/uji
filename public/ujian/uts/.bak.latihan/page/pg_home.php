...
//echo $id_user;exit();
	...
	/*if($_SESSION['851namauser']<>'' OR $_SESSION['851nama']<>'' OR $_SESSION['851group']<>'' ){
		...
		if($l_sess['testuser']<>"" or $l_sess['testname']<>"" or $l_sess['questionid']<>""){
			mysql_query("UPDATE ".$db_prefix."session SET testuser='',testname='',questionid='', nomor='' WHERE id_session='".$id_user."'");
		}
	/*} */
            ...
            <div class="user-profile-sidebar">
              <div class="row">
                ...
                <div class="col-md-12">
					...
					<a href="<?php echo $pathavatar;?>" class="thumbnail-view-hover image-zoom"> </a>
					<img alt="<?php echo $nama;?>" src="<?php echo $pathavatar;?>" style="max-width:150px;">
					</div></center></div>
				</div>
                <div class="col-md-12">
		...
		<table  border="0" cellpadding="0" cellspacing="0" width="100%"  class="table">
			...
			<?php 
				$q_krs=mysql_query("SELECT
				  a.testid, a.test_description, a.subjectid, a.test_datestart, a.test_dateend, a.test_type, a.test_attempts,
			...
			<?php } ?>
					<tr>
						...
						<?php }elseif($krs['test_datestart']<$unix_timestamp){
									...
									$num_tamp=mysql_fetch_array(mysql_query("select test_attempt_count  from ".$db_prefix."tests_attempts where testid='$krs[testid]' AND userid='$nim' "));
							...
							<?php 	}else{ ?>
										...
										<td>
											<form action="ujian-<?php echo base64_encode($id_user); ?>" method="post" style="margin:0;padding:0;" name="ujian">
											<input type="hidden" name="testid" value="<?php echo $krs['testid']; ?>">
											<input type="hidden" name="testname" value="<?php echo $krs['test_description']; ?>">
