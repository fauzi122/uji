                        ...
                        <ul class="app-actions">
                            <li>
                                <a href="#">
                                    ...
                                    @endforeach ||Pertemuan :    
                                    @foreach ($temu_ajar as $temu)
                                   {{$temu->pertemuan}}
                                    @endforeach
                                    || {{$tgl}}||<div id="clock"></div>||
            ...
            <div class="tab-pane fade" id="profile3" role="tabpanel" aria-labelledby="profile-tab3">
                        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
							<div class="card">
                                ...
                                <form method="post" action="/bap-ujian-gabung" enctype="multipart/form-data">
                                    @csrf
								<div class="card-body">
                ...
                <div class="row gutters justify-content-center">
                    <div class="col-xl-5 col-lg-5 col-md-6 col-sm-8 col-12">
                        ...
                        <div class="subscribe-form">
                            <form action="/absen-keluar-ujianG" method="POST">
            ...
            <div class="tab-pane fade show active" id="mhs3" role="tabpanel" aria-labelledby="mhs-tab3">
                ...
                <div class="row gutters justify-content-center">
                    <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
                        ...
                        <form action="/absenmhs-ujian-gabung" method="post">
                                    @csrf
                            ...
                            <div class="card-body p-0">
                                ...
                                @foreach ($mahasiswa as $nim=>$mhs)											
                                            <tr>
                                                    ...
                                                    <div class="project-details">
                                                        <div class="project-info">
                                                            <div class="status approved">
                                                                ...
                                                                $data=json_encode(isset($mhs_hadir[$nim]->sts) ? $mhs_hadir[$nim]->sts:'');
                                                               @endphp">
                                                                ...
                                                                <input type="hidden" name="no_urut[{{$count}}]" value="{{$nim}}"/>
                                                                    ...
                                                                    <div class="custom-control custom-radio custom-control-inline">
                                                                        <input type="radio" id="radio5[{{$count}}]" name="nama_radio[{{$count}}]" class="custom-control-input" value="1" 
                                                                        @if (json_decode($data) == '1')
                                                                            {{' checked="checked"'}}
                                                                        @endif
                                                                        >
                                                                        <label class="custom-control-label" for="radio5[{{$count}}]">Hadir</label>
                                                                    </div>
                                                                    <div class="custom-control custom-radio custom-control-inline">
                                                                        <input type="radio" id="radio4[{{$count}}]" name="nama_radio[{{$count}}]" class="custom-control-input" value="0"
                                                                        @if (json_decode($data) == '0' || json_decode($data) == '')
                                                                            {{' checked="checked"'}}
                                                                        @endif
                                                                        >
                                                                        <label class="custom-control-label" for="radio4[{{$count}}]">Tidak Hadir</label>
                                                                    </div>
                                                                    <div class="custom-control custom-radio custom-control-inline">
                                                                        <input type="radio" id="radio3[{{$count}}]" name="nama_radio[{{$count}}]" class="custom-control-input" value="2"
                                                                        @if (json_decode($data) == '2')
                                                                            {{' checked="checked"'}}
                                                                        @endif
                                                                        >
                                                                        <label class="custom-control-label" for="radio3[{{$count}}]">Bermasalah</label>
                                                                    </div>
                                                            </div>
                                                        </div>
                                                    </div>
                                                ...
                                                <td>
                                {{isset($mhs_sts[$nim]->ruang) ? $mhs_sts[$nim]->ruang:''}}
                                <input type="hidden" name="kd_lokal[{{$count}}]" value="{{isset($mhs_sts[$nim]->kelompok) ? $mhs_sts[$nim]->kelompok:''}}">
                                <input type="hidden" name="resultid[{{$count}}]" value="{{isset($mhs_hadir[$nim]->resultid) ? $mhs_hadir[$nim]->resultid:''}}">
                                <input type="hidden" name="kd_mtk[{{$count}}]" value="{{isset($mhs_sts[$nim]->kd_soal) ? $mhs_sts[$nim]->kd_soal:''}}">

                                                </td>
                                                <td>
                                    <textarea name="ket[{{$count}}]" cols="15" rows="1">{{isset($mhs_hadir[$nim]->ket) ? $mhs_hadir[$nim]->ket:''}}</textarea>
                                                </td>
                                            </tr>
                                            @php
                                            $count++;
