...
<div class="">
    ...
    <div class="row gutters">
        <div class="col-xl-12 col-lg-12 col-md-12 col-sm-12 col-12">
            ...
            $count=1;
            @endphp
            {{-- @if(isset($mhs_hadir[$nim]->tgl_hadir))$mhs_hadir[$nim]->tgl_hadir @else $tanggal @endif --}}
        @foreach ($mahasiswa as $nim=>$mhs)
        <input type="hidden" name="no_urut[{{$loop->iteration}}]" value="{{$nim}},{{$mhs->kd_lokal}},@if(isset($mhs_hadir[$nim]->tgl_hadir)){{$mhs_hadir[$nim]->tgl_hadir}}@else{{$tanggal}}@endif,@if(isset($mhs_hadir[$nim]->jam_hadir)){{$mhs_hadir[$nim]->jam_hadir}}@else{{$time}}@endif   
           
        "/>
        {{-- <input type="hidden" name="kd_lokal[{{$loop->iteration}}]" value="{{$mhs->kd_lokal}}">
            <input type="hidden" name="tgl_hadir[{{$loop->iteration}}]" value="
            @php
            ...
            @endphp
            ">
            <input type="hidden" name="jam_t[{{$loop->iteration}}]" value="@php
