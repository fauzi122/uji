            ...
            <div class="col-lg-4 col-md-4 col-sm-12">
                <div class="pricing-plan">
                    ...
                    @php
                        if($jad->kd_gabung<>''){
                        @endphp
                        <form action="/rekap-gabung-mbkm" method="post">
                        @csrf
                    ...
                    @php  }elseif($jad->kel_praktek==''){ 
                        @endphp
                        <form action="/rekap-teori-mbkm" method="post">
                            @csrf
                            <input type="hidden" name="kd_mtk" value="{{$jad->kd_mtk}}">
                            <input type="hidden" name="nm_mtk" value="{{$jad->nm_mtk}}">
                            <input type="hidden" name="kd_dosen" value="{{$jad->kd_dosen}}">
                     ...
                     <form action="/rekap-praktek" method="post">
                        @csrf
                        <input type="hidden" name="kd_mtk" value="{{$jad->kd_mtk}}">
                        <input type="hidden" name="nm_mtk" value="{{$jad->nm_mtk}}">
                        <input type="hidden" name="kd_dosen" value="{{$jad->kd_dosen}}">
                        ...
                        @endphp 
                        <button type="submit" class="btn btn-primary btn-lg">Rekap Absen</button>
